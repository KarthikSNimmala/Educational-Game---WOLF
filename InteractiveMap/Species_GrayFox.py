...
def information(): 
    import pygame
    import SpeciesMenu
    print("Gray Fox menu is running")
    ...
    title = title_font.render('Gray Fox' , True , color)
    ...
    WolfPark_heading = heading_font.render('Gray Foxes and Wolf Park', True, color)
    protect_heading = heading_font.render('How can people protect Gray Foxes?', True, color)
    ...
    while True: 
        for ev in pygame.event.get(): 
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN: 
                ...
                #if the mouse is clicked on the eigth button (Gray Foxes and Wolf Park)
                if 3*width/4-70 <= mouse[0] <= 3*width/4+200 and height/2-40 <= mouse[1] <= height/2: 
                    pygame.quit()
                
                #if the mouse is clicked on the ninth button (How can people protect gray foxes?)
