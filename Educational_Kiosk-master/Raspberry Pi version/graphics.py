# -*- coding: latin-1 -*-
# The above line enables the interpreter not to trip on accent marks.
...
def grab_animation(*folders): # Animations belong in the "Animations" folder.
    return pygame.image.load(os.path.join('Animations',*folders))
...
def get_menu_graphics(S_WIDTH,S_HEIGHT):
    ...
    if float(S_WIDTH) / S_HEIGHT > float(bgw) / bgh:
        bg = pygame.transform.scale(bg,(S_WIDTH,int(S_WIDTH*bgh/bgw)))
        bg = bg.subsurface(0,int(bg.get_height()/2-S_HEIGHT/2),S_WIDTH,int(bg.get_height()/2-S_HEIGHT/2)+S_HEIGHT)
    else:
            ...
            for orient in ORIENTATIONS:
                ...
                if orient != 1:
                    for each_frame in range(1,9):
                        wolfGraphics[orient].append(even_scale(grab_animation('Wolves',wolf,wolf+ORIENTATIONS[orient]+four_digit(each_frame)+'.png'),80))
                else:
                    for each_frame in range(1,9):
                        wolfGraphics[orient].append(even_scale(grab_animation('Wolves',wolf,wolf+ORIENTATIONS[orient]+four_digit(each_frame)+'.png'),96))
        ...
        for species in OTHER_ANIMALS:
            if OTHER_ANIMALS[species][1]:
                ...
                for frame in range(1,OTHER_ANIMALS[species][0]+1):
                    ...
                    if species == 'rabbit':
                        animalGraphics[species][0][-1] = even_scale(animalGraphics[species][0][-1],54)
                    elif species == 'deer':
                        animalGraphics[species][0][-1] = even_scale(animalGraphics[species][0][-1],120)
                    elif species == 'fox':
                        animalGraphics[species][0][-1] = even_scale(animalGraphics[species][0][-1],108)
                    elif species == 'bison':
                        animalGraphics[species][0][-1] = even_scale(animalGraphics[species][0][-1],160)
                    animalGraphics[species][1].append(pygame.transform.flip(animalGraphics[species][0][-1],True,False))
        return animalGraphics

    worldGraphics['animals'] = getAnimalGraphics()
