...
def play(chapter,character,BUTTONS,screen,menuGraphics,worldGraphics,timer,player=None):
    ...
    bliterate(population_screen.image,"Bison:  %d\n\nDeer:  %d\n\nFoxes:  %d\n\nRabbits:  %d\n\nWolves:  %d"%(play_world.population_dict['bison'],play_world.population_dict['deer'],play_world.population_dict['fox'],play_world.population_dict['rabbit'],play_world.population_dict['wolf']),S_WIDTH//48,S_WIDTH//48,S_WIDTH//8)
    ...
    while True:
        ...
        # Check which buttons are currently pressed, and find the player's
        # direction accordingly.
        up = BUTTONS["UP"].is_pressed
        down = BUTTONS["DOWN"].is_pressed
        left = BUTTONS["LEFT"].is_pressed
        right = BUTTONS["RIGHT"].is_pressed
        running = BUTTONS["GO"].is_pressed
        direction = -1
        if up and not down:
        ...
        for touched_obj in colliding:
            ...
            if isinstance(touched_obj,Moving) and not isinstance(touched_obj,Player):
                ...
                if isinstance(touched_obj,Wolf):
                    if since_talked > 60:
                        dialog(BUTTONS,screen,menuGraphics,"*sniff*",["*sniff*"],touched_obj.portrait)
                        since_talked = 0
                # If the player encounters a stronger creature (bison), player is dead.
                elif touched_obj.strength > player.strength:
                    dialog(BUTTONS,screen,menuGraphics,"YOU DIED",["Return to main menu"])
                    return 0
                ...
                # do not attack, because wolves do not hunt for sport.
                    ...
                    if since_talked > 60:
                        dialog(BUTTONS,screen,menuGraphics,"I'm not hungry enough to attack this animal.",["Leave it"],touched_obj.image)
        ...
        if chapter == 1:
            if play_world.age == 1:
                akela(BUTTONS,screen,menuGraphics,"Welcome to the world, %s!"%player_name,['Press the red button to continue.'])
                akela(BUTTONS,screen,menuGraphics,"I am Akela, leader of this pack.  We wolves form packs to help us survive.")
                needs_control = akela(BUTTONS,screen,menuGraphics,"Would you like a demonstration of the game's controls?",['No, thank you.','Yes, please!'])
                if needs_control == 1:
                    akela(BUTTONS,screen,menuGraphics,"Sure thing, %s!"%player_name)
                    akela(BUTTONS,screen,menuGraphics,"Use the arrow buttons to move around.")
            elif play_world.age == 150 and needs_control:
                akela(BUTTONS,screen,menuGraphics,"Hold the red button to run at your top speed.")
                since_talked = 0
            elif play_world.age == 300 and needs_control:
                akela(BUTTONS,screen,menuGraphics,"Explain health bar.")
                since_talked = 0
            elif since_talked > 180:
                contains = check_for_animals(visible)
                if not met['predator'] and contains['predator']:
                    akela(BUTTONS,screen,menuGraphics,"Look out!")
                    dialog(BUTTONS,screen,menuGraphics,"That's a bison.  They can trample a wolf.",["Wow, I'll look out, then."],contains['predator'].image)
                    met['predator'] = True
                    since_talked = 0
                elif not met['human'] and contains['human']:
                    akela(BUTTONS,screen,menuGraphics,"Be careful!")
                    dialog(BUTTONS,screen,menuGraphics,"Humans live nearby.",["Remark about humans."],contains['human'].image)
                    met['human'] = True
                    since_talked = 0
                elif not met['ignore'] and contains['ignore']:
                    akela(BUTTONS,screen,menuGraphics,"Do you see that?")
                    dialog(BUTTONS,screen,menuGraphics,"Those won't hurt us if we leave them alone - so we do.",["Okay"],contains['ignore'].image)
                    met['ignore'] = True
                    since_talked = 0
                elif not met['edge'] and not play_world.safe_focus_rect.collidepoint(player.rect.centerx,player.rect.centery):
                    akela(BUTTONS,screen,menuGraphics,"Be careful not to venture beyond the pack's territory.")
                    met['edge'] = True
                    since_talked = 0
                elif (not staying) and met['predator'] and met['prey'] and met['human'] and met['ignore'] and met['edge']:
                    akela(BUTTONS,screen,menuGraphics,"You've learned everything a pup should know.")
                    since_talked = 0
                    choice = dialog(BUTTONS,screen,menuGraphics,"What do you want to do?",['Form my own pack!','Stay with this pack for the rest of the year.','Return to the main menu.'],player.portrait)
                    if choice == 0:
                        return player
                    elif choice == 1:
                        akela(BUTTONS,screen,menuGraphics,"That's quite natural.  Most wolves don't leave the pack until they're two years old.")
                        staying = True
                    elif choice == 2:
                        return 0
                elif contains['prey'] and (not met['prey'] or player.health < 60):
                    akela(BUTTONS,screen,menuGraphics,"Looks like you found some food.")
                    since_talked = 0
                    if player.health > 60:
                        dialog(BUTTONS,screen,menuGraphics,"Do you want to hunt it?",["No, I'm not hungry."],contains['prey'].image)
                    else:
                        if dialog(BUTTONS,screen,menuGraphics,"Do you want to hunt it?",["No, I'm not hungry.","Yes, I am hungry."],contains['prey'].image):
                            akela(BUTTONS,screen,menuGraphics,"Then I will call the pack.")
                ...
                if play_world.age == 4*SEASON_LENGTH*DAY_LENGTH*FRAMES_PER_SECOND:
                    if staying:
                        akela(BUTTONS,screen,menuGraphics,"You're a year old, and have learned very much.")
                    else:
                        akela(BUTTONS,screen,menuGraphics,"You're a year old, having survived your first winter.")
                    choice = dialog(BUTTONS,screen,menuGraphics,"Do you want to leave the pack?",['Yes, I will form my own pack!','No, I want to stay.','Return to the main menu.'],player.portrait)
                    if choice == 0:
                        return player
                    elif choice == 1:
                        akela(BUTTONS,screen,menuGraphics,"Okay, you can stay with our pack another year.")
                    elif choice == 2:
                        return 0
                elif play_world.age == 8*SEASON_LENGTH*DAY_LENGTH*FRAMES_PER_SECOND:
                    akela(BUTTONS,screen,menuGraphics,"You're two years old now.  It's time you found a mate and formed your own pack.")
                    choice = dialog(BUTTONS,screen,menuGraphics,"What would you like to do?",["Form my own pack!","Return to the main menu."],player.portrait)
                    if choice == 0:
                        return player
                    else:
                        return 0
            ...
            if play_world.age == 1:
                akela(BUTTONS,screen,menuGraphics,"On behalf of the pack, I wish you well in your search for a new family.")
                akela(BUTTONS,screen,menuGraphics,"You'll be hunting alone now, until you find your new den.")
                dialog(BUTTONS,screen,menuGraphics,"Akela",["Dens look like this, and are usually found near water."],worldGraphics['misc']['den'])
                since_talked = 0
            elif not seen_wolf and check_for_animals(visible)["wolf"]:
                if since_talked > 30:
                    dialog(BUTTONS,screen,menuGraphics,"",['Hey, I think I see something!'],check_for_animals(visible)["wolf"].image)
                    seen_wolf = True 
                    since_talked = 0
            elif seen_wolf and check_for_animals(colliding)["wolf"]:
                obj = check_for_animals(colliding)['wolf']
                if isinstance(obj,Den):
                    dialog(BUTTONS,screen,menuGraphics,"",["Friendly howling"],player.portrait)
                    dialog(BUTTONS,screen,menuGraphics,"",["Friendly howling back"],menuGraphics['wolf_portrait_'+str(random.choice(range(len(menuGraphics['wolf_name_list']))))])
                else:
                    dialog(BUTTONS,screen,menuGraphics,"",["*friendly howling* Would you like to join our pack?"],obj.portrait)
                choice = dialog(BUTTONS,screen,menuGraphics,"What do you want to do?",["Join this pack!","Return to the main menu."],player.portrait)
                if choice == 0:
                    return player
                else:
                    return 0
            elif play_world.age == 4*SEASON_LENGTH*DAY_LENGTH*FRAMES_PER_SECOND:
                dialog(BUTTONS,screen,menuGraphics,"",["Winter came and went, and I never found a new pack.  I guess I'm a lone wolf forever."],player.portrait)
                dialog(BUTTONS,screen,menuGraphics,"",['Return to main menu'])
        else:
            if play_world.age == 1:
                dialog(BUTTONS,screen,menuGraphics,"Good morning, mate.",["*friendly howls*"],mate.portrait)
                dialog(BUTTONS,screen,menuGraphics,"Describe chapter 3 objectives",["Look up Grace's dialogue when Internet connection better."],play_world.mate.portrait)
            if play_world.age == 4*SEASON_LENGTH*DAY_LENGTH*FRAMES_PER_SECOND:
                dialog(BUTTONS,screen,menuGraphics,"YOU WIN",['Return to main menu'])
                return 0
