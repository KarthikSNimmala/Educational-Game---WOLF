...
class Terran(Entity):
    def __init__(self,appearance,night_appearance,pos,*groups):
        Entity.__init__(self,appearance,pos,*groups)
...
class Interactive(Terran):
    def __init__(self,appearance,night_appearance,pos,interactionBox,*groups):
        Terran.__init__(self,appearance,night_appearance,pos,*groups)
...
class Animated(Terran):
    def __init__(self,appearance,night_appearance,pos,fps,*groups):
        Terran.__init__(self,appearance[0],night_appearance[0],pos,*groups)
        self.appearance = appearance
        self.night_appearance = night_appearance
        self.fps = fps
        self.f = 0
    ...
    def update(self,why):
        if why == 'day':
            ...
            f = int(math.floor(why * self.fps / FRAMES_PER_SECOND) % len(self.appearance))
            if f != self.f:
                self.f = f 
...
class Stream(Animated):
    def __init__(self,appearance,night_appearance,pos,aim,*groups):
        Animated.__init__(self,appearance,night_appearance,pos,8,*groups)
        self.aim = aim 
        if len(aim) < 4:
...
class Moving(Animated):
    def __init__(self,appearance,night_appearance,pos,fps,max_speed,max_strength,collision_box,*groups):
        Animated.__init__(self,appearance[0],night_appearance[0],pos,fps,*groups)
        self.appearance = appearance
        self.night_appearance = night_appearance
    ...
    def update(self,why):
        if why == 'day':
            ...
            f = int(math.floor(why * self.fps / FRAMES_PER_SECOND + self.behind_frame) % len(self.appearance[self.orientation]))
...
class Player(Moving):
    def __init__(self,appearance,night_appearance,pos,portrait,*groups):
        Moving.__init__(self,appearance,night_appearance,pos,12,960,400,(0.2,0.8,0.6,0.9),*groups)
    ...
    def move(self,obstacles,direction):
        if direction != -1:
            Moving.move(self,obstacles,direction)

class Rabbit(Moving):
    def __init__(self,appearance,night_appearance,pos,*groups):
        self.direction = 0
        Moving.__init__(self,appearance,night_appearance,pos,12,640,50,(0.25,0.75,0.25,0.75),*groups)
    ...
    def move(self,obstacles,animals):
        ...
        if self.direction != -1:
            Moving.move(self,obstacles,self.direction)
...
class Deer(Moving):
    def __init__(self,appearance,night_appearance,pos,*groups):
        self.direction = 0
        Moving.__init__(self,appearance,night_appearance,pos,12,800,800,(0.25,0.75,0.25,0.75),*groups)
...
class Bison(Moving):
    def __init__(self,appearance,night_appearance,pos,*groups):
        self.direction = 0
        Moving.__init__(self,appearance,night_appearance,pos,12,1440,2000,(0.25,0.75,0.25,0.75),*groups)
    ...
    def move(self,obstacles,animals):
        ...
        if closest_dir != -1:
            Moving.move(self,obstacles,closest_dir)

class Wolf(Moving):
    def __init__(self,appearance,night_appearance,pos,portrait,*groups):
        Moving.__init__(self,appearance,night_appearance,pos,12,800,400,(0.2,0.8,0.6,0.9),*groups)
        self.portrait = portrait
        self.health = 100
...
class Fox(Moving):
    def __init__(self,appearance,night_appearance,pos,*groups):
        Moving.__init__(self,appearance,night_appearance,pos,12,960,200,(0.2,0.8,0.6,0.9),*groups)
...
class Obstacle(Terran):
    def __init__(self,appearance,nightappearance,pos,collision_box,*groups):
        Terran.__init__(self,appearance,nightappearance,pos,*groups)
        self.collision_box = collision_box
        self.collision_rect = compress_rect(self.image.get_rect(topleft=self.pos),self.collision_box)

class Rock(Obstacle):
    def __init__(self,appearance,nightappearance,pos,*groups):
        Obstacle.__init__(self,appearance,nightappearance,pos,(0.2,0.8,0.3,0.9),*groups)
...
class Tree(Obstacle): # The 'appearance' and 'evergreen' states should come from the getTreeGraphics dictionary.
    def __init__(self,appearance,nightappearance,pos,species,evergreen,*groups):
        Obstacle.__init__(self,appearance["summer"][0],nightappearance["summer"][0],pos,(0.4,0.6,0.7,0.9),*groups)
        self.appearance = appearance
    ...
    def update(self,why):
        if why in ['winter','autumn'] and not self.evergreen:
            ...
            t = int(math.floor(why * self.fps / FRAMES_PER_SECOND) % (8*m))
...
class Farm(Obstacle): # When in Internet, ensure super() refers to Obstacle!
    def __init__(self,appearance,nightappearance,pos,*groups):
        Obstacle.__init__(self,appearance,nightappearance,pos,(0.0,1.0,0.2,1.0),*groups)
        self.rect = compress_rect(self.image.get_rect(topleft = pos),(-0.3,1.3,-0.3,1.3))

class Den(Interactive):
    def __init__(self,appearance,nightappearance,pos,*groups):
        Interactive.__init__(self,appearance,nightappearance,pos,(0.25,0.75,0.25,0.75),*groups)
