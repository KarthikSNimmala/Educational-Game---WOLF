...
def funda_menu(BUTTONS,screen,menuGraphics,sprites,positions_list,row_length=1,default_position=0,usebg=True):
    ...
    BUTTON_STATUS = {}
    for b in BUTTONS:
        BUTTON_STATUS[b] = BUTTONS[b].is_pressed
    ...
    def change():
        ...
        pygame.display.update()
    while True:
        if BUTTONS["GO"].is_pressed:
            return p
        elif BUTTONS["UP"].is_pressed and not BUTTON_STATUS["UP"]:
            BUTTON_STATUS["UP"] = True
            if p > row_length-1:
                p -= row_length
                change()
        elif BUTTONS["DOWN"].is_pressed and not BUTTON_STATUS["DOWN"]:
            BUTTON_STATUS["DOWN"] = True
            if p < len(positions_list) - row_length:
                p += row_length
                change()
        elif BUTTONS["LEFT"].is_pressed and not BUTTON_STATUS["LEFT"]:
            BUTTON_STATUS["LEFT"] = True
            if p % row_length != 0:
                p -= 1
                change()
        elif BUTTONS["RIGHT"].is_pressed and not BUTTON_STATUS["RIGHT"]:
            BUTTON_STATUS["RIGHT"] = True
            if (p + 1) % row_length != 0:
                p += 1
                change()
        for b in BUTTONS:
            BUTTON_STATUS[b] = BUTTONS[b].is_pressed
        sleep(0.2) # Prevents unwanted double-pressing caused by imperfections in the buttons
...
def main_menu(screen,menuGraphics,BUTTONS):
    ...
    return funda_menu(BUTTONS,screen,menuGraphics,sprites,positions_list)
...
def char_select(screen,menuGraphics,BUTTONS):
    S_WIDTH, S_HEIGHT = screen.get_width(), screen.get_height()
    ...
    return wolf_choices[funda_menu(BUTTONS,screen,menuGraphics,sprites,positions_list,3,-1)]

def chapter_select(screen,menuGraphics,BUTTONS):
    S_WIDTH, S_HEIGHT = screen.get_width(), screen.get_height()
    ...
    return funda_menu(BUTTONS,screen,menuGraphics,sprites,positions_list,3)
...
def about_screen(screen,menuGraphics,BUTTONS):
    S_WIDTH, S_HEIGHT = screen.get_width(), screen.get_height()
...
def dialog(BUTTONS,screen,menuGraphics,question,options,image=None):
    S_WIDTH, S_HEIGHT = screen.get_width(), screen.get_height()
    ...
    return funda_menu(BUTTONS,screen,menuGraphics,sprites,positions_list,usebg=False)

# Special cases exist for dialog when Akela is speaking
def akela(BUTTONS,screen,menuGraphics,text,options=[]):
    if options == []:
        return dialog(BUTTONS,screen,menuGraphics,'Akela',[text],menuGraphics['akela'])
    else:
        return dialog(BUTTONS,screen,menuGraphics,text,options,menuGraphics['akela'])

# And for when the player is speaking
def selfnote(BUTTONS,screen,menuGraphics,text,portrait): # Usually portrait will be framelists[4]
    dialog(BUTTONS,screen,menuGraphics,'',[text],portrait)
